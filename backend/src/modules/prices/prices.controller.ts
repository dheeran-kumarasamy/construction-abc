...
export async function submitProductInquiry(req: Request, res: Response) {
  try {
    ...
    const requestedPhoneNumber = String(requested_phone_number || "").trim();
    const trimmedSpecification = String(specification || "").trim();

    if (!material_id || !district_id || requested_quantity == null || !trimmedSpecification || !requested_location || !requestedPhoneNumber) {
      return res.status(400).json({
        error: "material_id, district_id, requested_quantity, specification, requested_location, requested_phone_number are required",
      });
    }

    const quantity = Number(requested_quantity);
    if (!Number.isFinite(quantity) || quantity <= 0) {
      return res.status(400).json({ error: "0 should not be given in quantity or specification. requested_quantity must be greater than 0" });
    }

    if (/^0+(\.0+)?$/.test(trimmedSpecification)) {
      return res.status(400).json({ error: "0 should not be given in quantity or specification" });
    }

    const parsedQuotedPrice = quoted_price != null ? Number(quoted_price) : null;
    ...
    const data = await createProductInquiry(userId, {
      ...
      quoted_price: parsedQuotedPrice != null && Number.isFinite(parsedQuotedPrice) ? parsedQuotedPrice : null,
    });
